"""
Precomputed INTERP_REMESH weight replay for SSH boundary conditions.

Uses weights exported from the Fortran INTERP_REMESH subroutine to
produce numerically equivalent SSH interpolation without Delaunay
triangulation. The weights are computed once and stored in a .npz
file in the FIX directory.

Runtime:
1. First call: KDTree maps source coordinates to RTOFS grid cells (cached)
2. Per timestep: gather SSH at source cells, apply weights (pure indexed ops)
"""
